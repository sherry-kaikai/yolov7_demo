...
int main(int argc, char *argv[]){
  ...
  // initialize net
  YoloV7 yolov7(bm_ctx);
  CV_Assert(0 == yolov7.Init(
  ...
  // profiling
  TimeStamp yolov7_ts;
  TimeStamp *ts = &yolov7_ts;
  yolov7.enableProfile(&yolov7_ts);

  // get batch_size
  int batch_size = yolov7.batch_size();
  ...
  if (info.st_mode & S_IFDIR){
    ...
    for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++){
      ...
      if ((int)batch_imgs.size() == batch_size){
        // predict
        CV_Assert(0 == yolov7.Detect(batch_imgs, boxes));
...
#endif
            // draw image
              yolov7.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height, batch_imgs[i]);
      ...
      if ((int)batch_imgs.size() == batch_size) {
        ...
        for(int i = 0; i < batch_size; i++){
          ...
          for (auto bbox : boxes[i]) {
            ...
            cout << "  class id=" << bbox.class_id << ", score = " << bbox.score << " (x=" << bbox.x << ",y=" << bbox.y << ",w=" << bbox.width << ",h=" << bbox.height << ")" << endl;
#endif
            yolov7.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height, batch_imgs[i], false);
          }
  ...
  yolov7_ts.calbr_basetime(base_time);
  yolov7_ts.build_timeline("yolov7 test");
  yolov7_ts.show_summary("yolov7 test");
  yolov7_ts.clear();
