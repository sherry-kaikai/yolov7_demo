...
YoloV7::YoloV7(std::shared_ptr<BMNNContext> context):m_bmContext(context) {
  std::cout << "YoloV7 ctor .." << std::endl;
}

YoloV7::~YoloV7() {
  std::cout << "YoloV7 dtor ..." << std::endl;
...
int YoloV7::Init(float confThresh, float nmsThresh, const std::string& coco_names_file)
{
...
void YoloV7::enableProfile(TimeStamp *ts)
{
  m_ts = ts;
}

int YoloV7::batch_size() {
  return max_batch;
};

int YoloV7::Detect(const std::vector<bm_image>& input_images, std::vector<YoloV7BoxVec>& boxes)
{
  int ret = 0;
  //3. preprocess
...
int YoloV7::pre_process(const std::vector<bm_image>& images){
...
float YoloV7::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool *pIsAligWidth)
{
  float ratio;
...
int YoloV7::post_process(const std::vector<bm_image> &images, std::vector<YoloV7BoxVec>& detected_boxes)
{
  YoloV7BoxVec yolobox_vec;
  ...
  for(int batch_idx = 0; batch_idx < images.size(); ++ batch_idx)
  {
    ...
    std::vector<YoloV7BoxVec> class_vec(m_class_num);
...
int YoloV7::argmax(float* data, int num){
  float max_value = 0.0;
  int max_index = 0;
  for(int i = 0; i < num; ++i) {
    float value = data[i];
...
void YoloV7::NMS(YoloV7BoxVec &dets, float nmsConfidence)
{
  int length = dets.size();
  int index = length - 1;

  std::sort(dets.begin(), dets.end(), [](const YoloV7Box& a, const YoloV7Box& b) {
...
void YoloV7::drawPred(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
...
void YoloV7::draw_bmcv(bm_handle_t &handle, int classId, float conf, int left, int top, int width, int height, bm_image& frame, bool put_text_flag)   // Draw the predicted bounding box
{
